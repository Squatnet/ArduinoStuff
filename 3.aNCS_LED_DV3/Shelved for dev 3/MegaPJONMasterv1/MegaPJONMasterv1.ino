...
uint32_t t_millis;
String string = "";
const size_t bufferSize = 3*JSON_ARRAY_SIZE(4) + JSON_ARRAY_SIZE(7) + 15*JSON_OBJECT_SIZE(2) + JSON_OBJECT_SIZE(3);
  ...
  Dev["register"].printTo(Serial);
        ...
        if (Dev["register"] == "Strip"){
          ...
          if(singleDvc.success()){
            Serial.print("Added");
            const char * slag = Dev["name"].as<char*>();
            singleDvc["name"] = slag;
          ...
          JsonObject& singleDvc = serial.createNestedObject();
          if(singleDvc.success()){
            Serial.print("Added");
          ...
          JsonObject& singleDvc = matrix.createNestedObject();
          if(singleDvc.success()){
            Serial.print("Added");
            singleDvc["name"] = Dev["name"];
            singleDvc["id"] = senderId;
...
void pollDevice(uint8_t rid){
  ...
  const char * JSON = "{\"poll\":\" \"";
    ...
    bus.update();
    Serial.println(bus.send(rid,JSON,Size));
    bus.update();
