...
void loop() {
  if (millis() - t_millis > 10000) {
    ...
    Serial.println();
    const char * JSON = "MESSAGE"; // enter the json to describe this slave here, note the \" to escape
    int Size = 0;
    while (JSON[Size] != '\0') Size++;
    Serial.print("Registered With Master");
    bus.update();
    Serial.println("Sending packet to Device id 1");
    Serial.println(bus.send(1, JSON, Size));
    bus.update();
    Serial.println("Sending packet to Device id 2");
    Serial.println(bus.send(2, JSON, Size));
    bus.update();
    Serial.println("Sending packet to Broadcast");
    //Serial.println(bus.send(PJON_BROADCAST, JSON, Size));
