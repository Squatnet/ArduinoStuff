...
void loop() {
  ...
  if(string.startsWith("{")){
    ...
    const char json[string.length()+1];
    string.toCharArray(json,string.length()+1);
    JsonObject& root = jsonBuffer.parseObject(json);
    // {"Ptrn":2,"Secs":30,"Auto":true,"Col":"FFFFFF"}
    x = root["Ptrn"]; // 3
    autoSecs = root["Secs"]; // 25
    autoMode = root["Auto"].as<int>(); // true
    CRGB ourCol = root["Col"].as<char>();
  }
  if(string != "")string = "";
  FastLED.show();
