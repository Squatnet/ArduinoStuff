  ...
  if (millis() - t_millis > 10000) {
    ...
    for (uint8_t i = 0; i < PJON_MAX_DEVICES; i++) {
      if (bus.ids[i].state) {
        ...
        Serial.println(bus.send_repeatedly(1,"FUCKMOTHER",10,1));
        Serial.println("Sending packet to Device id 2");
        Serial.println(bus.send_packet(2,"FUCKMOTHER",10));
        Serial.println("Sending packet to Broadcast");
        Serial.println(bus.send_packet(PJON_BROADCAST,"FUCKMOTHER",10));
